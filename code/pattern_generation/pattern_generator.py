...
class MetaTile:
    ...
    def eval_child(self, n, i):
        return multiply_mat_vec(self.children[n].T, self.children[n].geom.shape[i])
...
def H_init():
    ...
    meta.add_child(mat_mul(create_trans_mat(2.5, y_axis_in_hex.y), mat_mul([-y_axis_in_hex.x,-y_axis_in_hex.y,0,y_axis_in_hex.y,-y_axis_in_hex.x,0], [y_axis_in_hex.x,0,0,0,-y_axis_in_hex.x,0])), H1_hat)
...
def P_init():
    ...
    meta.add_child(mat_mul(create_trans_mat(0, 2 * y_axis_in_hex.y), mat_mul([y_axis_in_hex.x, y_axis_in_hex.y, 0, -y_axis_in_hex.y, y_axis_in_hex.x, 0], [y_axis_in_hex.x, 0.0, 0.0, 0.0, y_axis_in_hex.x, 0.0])) ,P_hat)
...
def F_init():
    ...
    meta.add_child(mat_mul(create_trans_mat(0, 2 * y_axis_in_hex.y), mat_mul([y_axis_in_hex.x, y_axis_in_hex.y, 0, -y_axis_in_hex.y, y_axis_in_hex.x, 0], [y_axis_in_hex.x, 0.0, 0.0, 0.0, y_axis_in_hex.x, 0.0])), F_hat)
...
def construct_patch(H, T, P, F):
    ...
    for r in rules:
        if len(r) == 1:
            ...
            P = multiply_mat_vec(T, poly[(r[1]+1)%len(poly)])
            Q = multiply_mat_vec(T, poly[r[1]])
            nshp = shapes[r[2]]
            npoly = nshp.shape
            ...
            P = multiply_mat_vec(chQ.T, chQ.geom.shape[r[3]])
            Q = multiply_mat_vec(chP.T, chP.geom.shape[r[1]])
            nshp = shapes[r[4]]
            npoly = nshp.shape
...
def construct_metatiles(patch):
    ...
    new_P_outline = [p72, vec_add(p72, vec_subtract(bps1, llc)), bps1, llc]
    new_P = MetaTile(new_P_outline, patch.width * 2)
    for ch in [7,2,3,4,28]:
        new_P.add_child(patch.children[ch].T, patch.children[ch].geom)

    new_F_outline = [bps2, patch.eval_child(24, 2), patch.eval_child(25, 0), p252, vec_add(p252, vec_subtract( llc, bps1 ))]
    ...
    AAA = new_H_outline[2]
    BBB = vec_add(new_H_outline[1], vec_subtract( new_H_outline[4], new_H_outline[5]))
    CCC = multiply_mat_vec(create_rot_mat_around_point(BBB, -pi/3), AAA)
